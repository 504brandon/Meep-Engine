...
class PlayState extends MusicBeatState {
	...
	function startCountdown():Void {
		...
		startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer) {
			...
			introAssets.set('school', ['ready-pixel.png', 'set-pixel.png', '/intro/date-pixel.png']);
			introAssets.set('schoolEvil', ['ready-pixel.png', 'set-pixel.png', 'intro/date-pixel.png']);
	...
	private function generateSong(dataPath:String):Void {
		...
		for (section in noteData) {
			...
			for (songNotes in section.sectionNotes) {
				...
				var playSection:Bool = (section.mustHitSection != GameplayMenu.op);

				var gottaHitNote:Bool = ((songNotes[1] > 3) != playSection);
	...
	private function generateStaticArrows(player:Int):Void {
		for (i in 0...4) {
			...
			switch (curStage) {
				...
				default:
					babyArrow.frames = FlxAtlasFrames.fromSparrow('assets/images/ui/maingame/arrows/NOTE_assets.png',
						'assets/images/ui/maingame/arrows/NOTE_assets.xml');
	...
	override public function update(elapsed:Float) {
		...
		if (health < 0 && GameplayMenu.op)
			health = 0;

		if (health > 2 && !GameplayMenu.op)
			health = 2;
		...
		} else if (health <= 0 && !GameplayMenu.op) {
			boyfriend.stunned = true;
...
function goodNoteHit(note:Note):Void {
	if (!note.wasGoodHit) {
		...
		if (GameplayMenu.op) {
			if (note.noteData >= 0)
				health -= 0.023;
		} else if (note.noteData >= 0 && !GameplayMenu.op){
				health += 0.023;
		}
